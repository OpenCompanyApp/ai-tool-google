...
class GoogleSheetsAddFilter implements Tool
{
    ...
    public function handle(Request $request): string
    {
        try {
            ...
            $range = $request['range'] ?? '';
            if (empty($range)) {
                return 'Error: range is required (A1 notation including header row).';
            }
            ...
            return "Filter applied to range '{$range}'.";
        } catch (\Throwable $e) {
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'range' => $schema
                ->string()
                ->description('A1 notation range including header row (e.g., "Sheet1!A1:D10").')
                ->required(),
        ];
    }
}
